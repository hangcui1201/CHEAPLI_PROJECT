...
float accelxSide = 1.494;//1.446;//1.4322; //1.4435;//0.7180;       //accel x offset for when starting on the side
float accelxSideStart = 1.494;//1.4322; //1.4435;//0.7180;
    ...
    if (!SIMU_beginnewdata) {// Only true if have not yet begun a message
        if (SIMU_checkfirstcommandbyte == 1) {
            ...
            if (0x7F == (unsigned char)data) {// Check for start char
                ...
                SIMU_Var3_toSIMU_16bit = 1000*vel;
